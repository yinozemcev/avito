...
def getResult(IDs, session):
    ...
    страницу с объявлением и номер телефона, в случае неудачи
    ...
    for ID in IDs:
        ...
        try:
            ...
            phoneResponse = getIDPhone(ID, key, session)
            if (phoneResponse['status'] == 'ok'):
                uri = phoneResponse['result']['action']['uri']
                #телефон представлен последними 11 символами в URI
                resultID['phone'] = uri[-11:]
            else:
                log(phoneResponse)
            #для получения телефонов приходится ждать 15 секунд, иначе банит сессию (без телефона достаточно задержки между запросами, которая создается работой программы)
            time.sleep(15)
...
def getIDPhone(ID, key, session):
    """
    Метод получения телефона по ID объявления
    ...
    """
    try:
        response = session.get('https://m.avito.ru/api/1/items/{}/phone'.format(ID),
                                params={'key': key}, timeout = 10.0).json()
    except requests.exceptions.Timeout:
        log('Page timed out (ID {})'.format(ID))
        response = session.get('https://m.avito.ru/api/1/items/{}'.format(ID),
                                params={'key': key}).json()
    return response
...
class HttpHandler(BaseHTTPRequestHandler):
    ...
    def parse(self, query, callback, proxies):
        ...
        и собирает нужную информацию (отдельным запросом
        также получает телефон)
        """
